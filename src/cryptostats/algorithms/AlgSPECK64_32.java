...
public class AlgSPECK64_32 implements ValidAlgorithm{
	 ...
	 */
	private SpeckEngine engine = new SpeckEngine(32);
	private FileAndTimer fat = new FileAndTimer();
	
	public AlgSPECK64_32(){
        fat.testTimer =  new TestTimer();

	}
	@Override
	public void generateRandomKeySet() {
		byte[] tempKey = FileGeneratorHelper.generateRandomByteArray(64/8);
	...
	public FileAndTimer encryptFile(File fileIn) {
		...
		try {
	        ...
	        for(int i = 0; ((i+1)*4) <= inputBytes.length; i++){
	        	engine.processBlock(inputBytes,(i*4),outputBytes,(i*4));
	        }      
