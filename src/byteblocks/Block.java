...
public class Block {
	...
	public static void main(String[] args){
		byte[] theNumber = {(byte) 0x00,(byte) 0x00,(byte) 0x01};
		char testChar = 255;
	...
	public void wrapRotateLeft(){
		...
		System.out.println("The most significant bit for the current rotation is: " + msb); 
		...
		for(int i = 0; i < length;i++){
			...
			if (i == length-1){
				System.out.println("We are currently on the nth block"); 
				block[i] = ByteOps.setBitNumber(block[i],0,msb);
			} else { 
				//We are just doing the first bytes so we get the msb of the next byte
				block[i] = ByteOps.setBitNumber(block[i],0,ByteOps.getBitNumber(block[i+1], 7));
			}
